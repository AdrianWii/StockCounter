<?php
	...
	if(isset($_POST['data'])){
		$data=$_POST['data'];
		$getdata=$mysqlZapytanie->zapytanie("CALL previewDailyReportFrom('$data')");
	
                /*$getdata=$mysqlZapytanie->zapytanie("select i.data_inwentaryzacji, CONCAT(p.nazwa,' ', q.pojemnosc, j.jednostka), SUM(i.ilosc) from inwentaryzacja i inner join sklad s on i.id_skladu=s.id_skladu inner join produkty p on s.id_produktu=p.id_produktu 
inner join  smaki t on p.id_smaku=t.id_smaku 
inner join jednostki j on p.id_jednostki = j.id_jednostki
inner join pojemnosci q on p.id_pojemnosci = q.id_pojemnosci where data_inwentaryzacji = '$data' GROUP BY p.nazwa");
       */ }

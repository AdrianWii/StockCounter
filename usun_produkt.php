<?php
	...
	$pyt=$mysqlZapytanie->zapytanie("select id_produktu, nazwa, smak, pojemnosc from product_details");
?>
...
<!--- duży div bez nawigacji ------------------ -->	
		...
		<div id="big_wrapper_wo_nav">
			...
			<!-- panel lewy -->
				...
				<form action="controller.php?method=deleteProduct" method="post">
					
					<span id=error><?php message(); ?></span>
					<table class="formularz_data">
						<tr class=input_login>
						<td class=label_delete id=label>WYBIERZ PRODUKT,<br>KTÓRY CHCESZ USUNĄĆ</td><td class=input_decoration>
											<select class="data" name="usun">
											<?php optionInject3($pyt); ?>
											</select></td>
						</tr>
						<tr class=input_login>
